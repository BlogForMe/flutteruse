...
class ShowMoreTextPopup {
  ...
  Offset _calculateOffset(BuildContext context) {
    double dx = _showRect.left + _showRect.width / 2.0 - _popupWidth / 2.0; //弹出框x坐标 = showmore按钮的 左边坐标x + showmore width - 弹出框/2
    if (dx < 10.0) {
      dx = 10.0;
    }
    ...
    double dy = _showRect.top - _popupHeight;   //弹出框顶部y坐标
    // if (dy <= MediaQuery.of(context).padding.top + 10) { //弹出框y坐标小于 箭头高度 就显示在控件下面
